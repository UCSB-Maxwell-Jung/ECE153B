...
void sccb_bus_init(void)
{
	...
	/*[TODO] RCC->APB2ER = GPIOB */ 						//1
	GPIO_InitStructure.GPIO_Pin = GPIO_PIN_6;				//2 
 	
	/*[TODO] Not sure how to set up internal pull up in step 3 */ 
	GPIO_InitStructure.GPIO_Mode = GPIO_MODE_OUTPUT_OD; 	//3	 set to output mode and use internal pull up		
	GPIO_InitStructure.GPIO_Pull = GPIO_PULLUP;

 	GPIO_InitStructure.GPIO_Speed = GPIO_SPEED_FREQ_HIGH;	//4
 	GPIO_Init(GPIOB, &GPIO_InitStructure);					//5
 	GPIO_SetBits(GPIOB, GPIO_PIN_6);						//6	using 

	/*___Serial Clk Line (SCL)___*/
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;				//7
 	GPIO_InitStructure.GPIO_Mode = GPIO_MODE_OUTPUT_PP; 	//8 set to Pull up pull down
 	GPIO_Init(GPIOB, &GPIO_InitStructure);					//9
 	GPIO_SetBits(GPIOB, GPIO_PIN_7);						//10
